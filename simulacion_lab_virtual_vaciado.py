...
# Estilo CSS personalizado: Estética UCV Profesional Completa
st.markdown("""
    <style>
...
# Encabezado Institucional con validación de archivos
...
# --- 3. BARRA LATERAL: PARÁMETROS COMPLETOS ---
...
def simular_paso(dt, h_prev, sp, geom, r, h_t, m_p, h_p, t_p, err_acum, err_prev):
    # Cálculo del área transversal según la geometría del tanque
    if geom == "Cilíndrico": 
        A_h = np.pi * (r**2)
    elif geom == "Cónico": 
        A_h = np.pi * ( (r / h_t) * h_prev )**2
    else: # Esférico
        A_h = np.pi * (2 * r * h_prev - h_prev**2)
    A_h = max(A_h, 0.01) 
...
if btn_simular:
    ...
    for i, t in enumerate(tiempo_anim):
        ...
        # 1. Esquema del Tanque
        fig_t, ax_t = plt.subplots(figsize=(4, 4))
        ax_t.set_xlim(-1.2*radio_max, 1.2*radio_max); ax_t.set_ylim(0, 1.1*altura_total)
        ...
        if geometria == "Cilíndrico":
            ...
            ax_t.add_patch(rect_w); ax_t.plot([-radio_max, -radio_max, radio_max, radio_max], [altura_total, 0, 0, altura_total], color='#2c3e50', lw=3)
        ...
        ax_t.axhline(y=setpoint, color='#e74c3c', ls='--', lw=2)
        tanque_plot.pyplot(fig_t); plt.close(fig_t)
        ...
        if hay_p and t >= t_p: ax_c.axvline(x=t_p, color='orange', ls=':', label="Perturbación")
        ax_c.set_xlim(0, t_sim); ax_c.set_ylim(0, 1.1*altura_total); ax_c.legend(loc="lower right"); ax_c.grid(True, alpha=0.3)
        ...
        if len(h_anim) > 1:
            ...
            metrica_r2.metric("R² Score", f"{r2_rt:.4f}"); metrica_mse.metric("MSE", f"{mse_rt:.6f}")
        
        tabla_resumen.table(pd.DataFrame({"Variable": ["Tiempo", "Qp", "Régimen"], "Valor": [f"{t} s", f"{p_act:.4f}", f"{tipo_proceso}"]}))
        time.sleep(0.01); progress_bar.progress((i + 1) / len(tiempo_anim))

    # --- 5. RESULTADOS FINALES ---
    boton_descarga.download_button(label="📥 Descargar Reporte CSV", data=pd.DataFrame({"t":tiempo_anim, "h":h_anim, "u":u_anim}).to_csv(index=False), file_name=f"sim_ucv_{geometria.lower()}.csv", use_container_width=True)
    st.success("✨ Ensayo culminado con éxito. Datos listos para el informe.")
else:
    st.info("💡 Configure los parámetros en la barra lateral e inicie el ensayo experimental.")
