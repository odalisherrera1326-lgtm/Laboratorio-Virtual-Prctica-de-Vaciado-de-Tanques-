...
# --- 5. BASE TEÓRICA ---
with st.expander("📚 Fundamentos Teóricos", expanded=True):
    col_a, col_b = st.columns([2, 1])
    col_a.write(f"Simulación de **{modo}** para un tanque **{tipo_tanque}**.")
    col_b.latex(r"\frac{dh}{dt} = \frac{Q_{in} - Q_{out}}{A(h)}")

# --- 6. FUNCIONES Y SIMULACIÓN ---
def calc_area(h):
...
if st.button("🚀 Iniciar Simulación"):
    ...
    ax1.plot(t_sim, h_hist, color="#00FF41", label="Simulado (PV)", lw=2)
    ax1.scatter(t_exp, h_exp, color="red", label="Experimental", s=15)
    ax1.axhline(sp, color="orange", ls="--", label="Setpoint")
    ax1.set_ylabel("Nivel (m)")
    ax1.legend()
    
    ax2.step(t_sim, u_hist, color="#00BFFF", label="Controlador (u)")
    ax2.set_ylabel("Válvula (u)")
    ...
    ss_res = np.sum((h_exp - h_interp)**2)
    ss_tot = np.sum((h_exp - np.mean(h_exp))**2)
    r2 = 1 - (ss_res / ss_tot)

    st.write("### 📊 Análisis")
    st.info(f"Precisión R²: {r2:.4f} | Nivel Final: {h_hist[-1]:.3f} m")
